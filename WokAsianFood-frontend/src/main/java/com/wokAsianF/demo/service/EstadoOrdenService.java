...
public class EstadoOrdenService {
    ...
    /**
     * 🎯 MÉTODO CLAVE: Actualiza el estado de la orden basándose en el progreso de
     * los platillos
    ...
    public boolean marcarListaParaPago(Integer ordenId) {
        ...
        // 🚨 Validación clave: Solo se puede enviar a caja si ya ha sido SERVIDA.
        if (orden.getEstadoOrden() != EstadoOrden.servida) {
            throw new IllegalStateException(
                    "La orden debe estar en estado 'servida' para enviarla a caja." +
                            "Estado actual: " + orden.getEstadoOrden());
        }

        orden.setEstadoOrden(EstadoOrden.LISTA_PARA_PAGO); // 🎯 ESTADO FINAL PARA CAJA
        ordenRepository.save(orden);
